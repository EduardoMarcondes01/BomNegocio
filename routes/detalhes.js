...
// âœ… FUNÃ‡Ã•ES DE BANCO DE DADOS
async function _getSellerInfo(vendedorId) {
  if (!isValidUUID(vendedorId)) return _getDefaultSeller(vendedorId);
  ...
  try {
    ...
    if (error || !vendedor) {
      return _getDefaultSeller(vendedorId);
    }
...
async function _checkFavoriteStatus(produtoId, usuarioId) {
  if (!isValidNumber(produtoId)) return false;
...
async function _checkLikeStatus(itemId, tipoConteudo, usuarioId) {
  if (!itemId || !tipoConteudo) return false;
...
async function _getLikesCount(itemId, tipoConteudo) {
  if (!itemId || !tipoConteudo) return 0;

  const itemIdNum = parseInt(itemId);
  if (!isValidNumber(itemId)) return 0;
...
async function _getAnuncios(categoria, usuarioId, cidade = null) {
  try {
    ...
    if (!propagandas || propagandas.length === 0) {
      return [];
    }

    const maisRecentes = propagandas.slice(0, 2);
    const restantes = propagandas.slice(2);
    ...
    if (anunciosFinal.length > 0) {
      ...
      const vendedoresMap = await _getUsersBatch(vendedorIds, 'id, nome, bairro, imagem_url, telefone');

      return anunciosFinal.map(anuncio => ({
        ...anuncio,
        vendedor: vendedoresMap.get(anuncio.usuario_id) || _getDefaultSeller(anuncio.usuario_id)
      }));
    }
...
async function _getComentarios(itemId, tipoConteudo) {
  const itemIdNum = parseInt(itemId);
  if (!isValidNumber(itemId)) return [];
...
// âœ… BUSCA DE PRODUTOS RECOMENDADOS
async function _getRecommendedProducts(categoria, usuarioId, cidade = null) {
  try {
    let query = supabase
      .from('produtos_propaganda')
    ...
    const restantes = propagandas.slice(2);
    const aleatorios = _shuffleArray(restantes).slice(0, 2);
    const produtosSelecionados = [...maisRecentes, ...aleatorios];
    const produtosFinal = produtosSelecionados.slice(0, 4);

    const vendedorIds = produtosFinal.map(p => p.usuario_id).filter(id => id);
    const vendedoresMap = await _getUsersBatch(vendedorIds, 'id, nome, bairro, imagem_url, telefone');

    return produtosFinal.map(produto => ({
      ...produto,
      vendedor: vendedoresMap.get(produto.usuario_id) || _getDefaultSeller(produto.usuario_id)
    }));
...
router.get('/initial-data', verifyToken, async (req, res) => {
  try {
    const { produto_id, tipo_conteudo, categoria, vendedor_id, cidade } = req.query;
    const usuario_id = req.user.id;

    if (!produto_id || !tipo_conteudo || !vendedor_id) {
      return ResponseHandler.validationError(res,
        'Dados obrigatÃ³rios nÃ£o fornecidos: produto_id, tipo_conteudo, vendedor_id'
      );
    }

    if (!isValidUUID(vendedor_id) || !isValidNumber(produto_id)) {
    ...
    const dataPromise = Promise.allSettled([
      _getSellerInfo(vendedor_id),
      _checkFavoriteStatus(produto_id, usuario_id),
      _checkLikeStatus(produto_id, tipo_conteudo, usuario_id),
      _getLikesCount(produto_id, tipo_conteudo),
      _getAnuncios(categoria, usuario_id, cidade),
      _getComentarios(produto_id, tipo_conteudo),
      _getRecommendedProducts(categoria, usuario_id, cidade)
    ]);

    const results = await Promise.race([dataPromise, timeoutPromise]);

    const responseData = {
      vendedor: _getResult(results[0], _getDefaultSeller(vendedor_id)),
      ...
      produtosRecomendados: _getResult(results[6], [])
    };
    ...
    if (error.message === 'TIMEOUT') {
      ...
      ResponseHandler.serverError(res, 'Erro ao carregar dados iniciais');
    }
  }
});
...
router.post('/visualizacoes/registrar', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!tipo_conteudo || !['produto', 'novidade', 'oferta', 'servico', 'propaganda'].includes(tipo_conteudo)) {
...
router.post('/mensagens', verifyToken, async (req, res) => {
  try {
    const { produto_id, destinatario_id, mensagem } = req.body;
    const remetente_id = req.user.id;

    if (!produto_id || !isValidNumber(produto_id)) {
      return ResponseHandler.validationError(res, 'ID do produto Ã© obrigatÃ³rio e deve ser vÃ¡lido');
    }
    ...
    if (errorProduto || !produto) {
      return ResponseHandler.validationError(res, 'Produto nÃ£o encontrado');
    }
    ...
    if (insertError) {
      if (insertError.code === '23503') {
        return ResponseHandler.validationError(res, 'Erro de referÃªncia - verifique IDs do produto e destinatÃ¡rio');
      }
      throw insertError;
    }
...
// âœ… FAVORITOS
router.get('/favoritos/verificar/:produtoId', verifyToken, async (req, res) => {
  try {
    const { produtoId } = req.params;
    const userId = req.user.id;

    if (!produtoId || !isValidNumber(produtoId)) {
      return ResponseHandler.validationError(res, 'ID do produto invÃ¡lido');
    }
    ...
    ResponseHandler.serverError(res, 'Erro ao verificar favorito');
  }
});
...
router.post('/favoritos', verifyToken, async (req, res) => {
  try {
    ...
    ResponseHandler.serverError(res, 'Erro ao atualizar favorito');
  }
});
...
router.post('/curtir', verifyToken, async (req, res) => {
  try {
    ...
    const { data: curtidas } = await supabase
      .from('curtidas')
      .select('id, usuario_id, produto_id, servico_id, novidade_id, tipo_conteudo')
    ...
    if (error.code === '42501') {
      ...
      ResponseHandler.serverError(res, 'Erro ao processar curtida');
    }
  }
});
...
router.get('/curtidas/verificar/:itemId', verifyToken, async (req, res) => {
  try {
    ...
    ResponseHandler.serverError(res, 'Erro ao verificar curtida');
  }
});
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        return ResponseHandler.success(res, { vendedor: _getDefaultSeller(id) });
      }
      throw error;
    }

    ResponseHandler.success(res, { vendedor: vendedor || _getDefaultSeller(id) });
...
// âœ… ENDPOINT DE PROPAGANDAS RECOMENDADAS
router.get('/propaganda', verifyToken, async (req, res) => {
  try {
    ...
    if (propagandas && propagandas.length > 0) {
      ...
      const restantes = propagandas.slice(2);
      const aleatorios = _shuffleArray(restantes).slice(0, 2);
      const propagandasSelecionadas = [...maisRecentes, ...aleatorios];
      const propagandasFinal = propagandasSelecionadas.slice(0, 4);

      const vendedorIds = propagandasFinal.map(p => p.usuario_id).filter(id => id);
      const vendedoresMap = await _getUsersBatch(vendedorIds, 'id, nome, bairro, imagem_url, telefone');

      const propagandasComVendedor = propagandasFinal.map(propaganda => ({
        ...propaganda,
        vendedor: vendedoresMap.get(propaganda.usuario_id) || _getDefaultSeller(propaganda.usuario_id)
      }));

      return ResponseHandler.success(res, {
        produtos: propagandasComVendedor,
        total: propagandasComVendedor.length,
        tipo: 'standard (2 recentes + 2 aleatÃ³rios)'
      });
    }
    ...
    ResponseHandler.serverError(res, 'Erro ao carregar propagandas');
  }
});
...
router.get('/fretes', verifyToken, async (req, res) => {
  try {
    ...
    if (servicos && servicos.length > 0) {
      ...
      const usuariosMap = await _getUsersBatch(usuarioIds, 'id, nome, bairro, imagem_url, telefone');
    ...
    ResponseHandler.serverError(res, 'Erro ao carregar serviÃ§os de frete');
  }
});
...
router.get('/subcategorias/:categoria', async (req, res) => {
  try {
    ...
    ResponseHandler.serverError(res, 'Erro ao carregar subcategorias');
  }
});
  ...
  try {
    ...
    if (propagandas && propagandas.length > 0) {
      ...
      return ResponseHandler.success(res, {
        ...
        tipo: 'basic (2 recentes + 2 aleatÃ³rios)'
      });
    }
    ...
    ResponseHandler.serverError(res, 'Erro ao carregar anÃºncios');
  }
});
